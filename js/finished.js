...
(function () {
  ...
  function makeScatterPlot(csvData) {
    ...
    d3.map(data, function (d) { return d.location; }).keys().forEach(val => {
      countries.set(val, color(val));
    });
    ...
    var legend = svgContainer.selectAll("g")
      .data(Array.from(countries.keys()))
      .enter().append("g")
      .attr("transform", function (d, i) { return "translate(" + (-1300 + i * 50) +", " + 525  +")"; });

    // the legend color guide
    legend.append("rect")
      .attr('height', 25)
      .attr('width', 12)
      .style("fill", function (d) { return countries.get(d); })
  ...
  function plotData(map) {
    ...
    svgContainer.selectAll('.dot')
      ...
      .attr('fill', (d) => countries.get(d.location))
      .style('opacity', .6)
      .attr("stroke-width", .5)
      .attr("stroke", "black")
